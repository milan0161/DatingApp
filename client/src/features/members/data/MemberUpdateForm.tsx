...
const UpdateMemberForm = ({
  ...
  return (
    <form id="editForm" onSubmit={handleSubmit(updateMemberHandler)}>
      ...
      <textarea className="w-full border border-slate-300 rounded p-2" rows={6} {...register('lookingFor')}></textarea>
      <h4 className="text-[30px] text-bold mb-4">Interests</h4>
      <textarea className="w-full border border-slate-300 rounded p-2" rows={6} {...register('interests')}></textarea>
      <h4 className="text-[30px] text-bold mb-4">Location Details</h4>
      <div className="flex items-center gap-x-4">
        <label htmlFor="city">City:</label>
        <input className="border border-slate-300 rounded p-2" type="text" {...register('city')} id="city" />
        <label htmlFor="country">Country:</label>
        <input className=" border border-slate-300 rounded p-2" type="text" {...register('country')} id="country" />
      </div>
    </form>
  );
};
